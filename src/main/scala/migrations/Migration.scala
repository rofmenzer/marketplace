...
class RunMigrations(db: Database) extends LazyLogging {
    ...
    val migrationList: List[Migration] = List(
        ...
        new MigrationCreateTableBook(db),
        new MigrationCreateTableOrders(db),
        new MigrationShipping(db),
        new MigrationPayment(db)
    )
    ...
    def apply() {
        ...
        migrationList.slice(1, migrationList.length).foreach(migration => {
            migration()
